...
export class AWSVoiceService {
  ...
  async synthesize(
    ...
    return this.withRetry(async () => {
      ...
      console.log('[Nova 2 Sonic] Synthesizing speech from text...');
      
      try {
        // Use Nova 2 Sonic for text-to-speech
        const command = new InvokeModelWithResponseStreamCommand({
          modelId: this.NOVA_SONIC_MODEL_ID,
          contentType: 'application/json',
          accept: 'application/json',
          body: JSON.stringify({
            ...
            system: [
              {
                text: 'You are a helpful assistant. Respond naturally and conversationally.'
              }
            ],
            inferenceConfig: {
              maxTokens: 1024,
        ...
        for await (const chunk of response.body) {
          if (chunk.chunk?.bytes) {
            ...
            if (chunkData.message?.content) {
              for (const content of chunkData.message.content) {
                if (content.audio?.data) {
                  audioChunks.push(Buffer.from(content.audio.data, 'base64'));
                }
              }
            }
          }
        ...
        if (audioChunks.length === 0) {
          // If no audio is generated, create a simple error message audio
        ...
        // Return empty buffer instead of throwing error to prevent audio playback failures
