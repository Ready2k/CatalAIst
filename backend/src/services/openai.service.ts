...
export class OpenAIService implements ILLMProvider {
  ...
  // Fallback models list (used when API call fails)
  ...
  async synthesize(
    ...
    // Validate and cast voice to allowed type
    const validVoices = ['alloy', 'echo', 'fable', 'onyx', 'nova', 'shimmer'] as const;
    type VoiceType = typeof validVoices[number];
    const selectedVoice: VoiceType = validVoices.includes(voice as VoiceType) 
      ? (voice as VoiceType) 
      : 'alloy';
    ...
    try {
      return await this.withRetry(async () => {
        ...
        return response.data.map((model) => ({
          ...
          supportsOnDemand: true, // OpenAI models are always on-demand
      ...
      console.warn('[OpenAI] Falling back to static model list');
  ...
  private getFallbackModels(): ModelInfo[] {
    return this.FALLBACK_MODELS.map((modelId, index) => ({
      id: modelId,
   ...
   * Accept any model that looks like an OpenAI model
   */
  isModelSupported(model: string): boolean {
    // Accept any model that starts with known OpenAI prefixes
    // This allows for new models without requiring code updates
    return (
      ...
      // Accept any model that contains common OpenAI patterns
      /^(gpt|o1|text|davinci|curie|babbage|ada)/.test(model)
    );
  }
